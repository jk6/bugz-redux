...
class TicketHistoryDisplay extends Component {
  ...
  render() {
    ...
    let ticketsDisplay = tickets.map((ticket, i) => {
      let appName = apps
        .filter((app) => app.id == ticket.appId)
        .map((app) => app.name)[0];
      ...
      let totalTextLength =
        Number(String(appName).length) + Number(String(ticket.issue.length));
      let availIssueLength = totalLength - Number(String(appName).length);

      let displayIssue =
        totalTextLength <= totalLength
          ? ticket.issue
          : `${ticket.issue.substr(0, availIssueLength)}...`;
      ...
      return (
        <li key={i} className="list-group-item" onClick={boundClick}>
          {display}
        </li>
      );
    });
